...
16. チーム稼働率（今週実稼働率 / 来週予定稼働率）
17. 主要指標の前週比（SPI / CPI / バグ / 期限超過）

## 表示要件
- テスト関連（不具合管理、品質、テスト品質）は「テストグループ」としてまとめる。
- HTML反映時にテストグループは折りたたみ/展開可能（details/summary）であることを前提に、weekly_report上で該当データを近接配置する。
- ブロッカー/外部依存、技術的負債、意思決定・エスカレーションは表示対象外とする。
- 静的HTMLは単一ファイルとし、週次報告タブ/開発メンバー向け進捗会議タブで切り替える。

## UIマッピング用追記事項（再現性向上）
weekly_report_YYYY-MM-DD.md の末尾に以下の「UIマッピング補助ブロック」を必ず付与する。

```markdown
## UI_MOCK_MAPPING
- HEADER_SUMMARY: 1文要約
- KPI: 完了,新規,未完了,バグ,期限超過,工数,SPI,CPI
- DOMAIN: タスク,内部,外部,リスク,変更
- TEST_GROUP: 不具合管理,品質,テスト品質
- PROGRESS_COST_RISK: 進捗,コスト,リスク
- NEXT_ACTIONS: 最大3件
- OVERDUE_TABLE: 期日超過チケット
- QUALITATIVE: 楽観,悲観,残業要否,要員追加要否
- MEMBER_TABLE: 今週SPI,予定/実績/来週予定,割込時間/理由,来週チケット
```
...
## ルール
- dev_progress_meeting: 開発メンバーが割込情報をリアルタイム入力可能な形式。
- weekly_report: 上長向けの参照表示のみ。
- 両ファイル先頭に対象期間と SNAPSHOT_AT を明記。
- 事実と解釈を分離。
- 次週アクションは最大3件。
- SPI/CPI根拠式（EV/PV, EV/AC）を必ず記載。
